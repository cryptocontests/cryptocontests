...
import { CryptoValue, Currency, CryptoCurrency, Fiat } from '../transaction.model';
...
export class CurrencyService {
  ...
  constructor(private http: HttpClient) {
    this.http.get(this.apiUrl).subscribe(response => this.conversions = response);
   }
  ...
  public cryptoToFiat(value: CryptoValue, returnCurrency: Fiat): number {
    ...
    if (CryptoCurrency[value.currency] === CryptoCurrency.BTC) {
      return this.conversions.data['1'].quotes[returnCurrency].price * value.value;
    } else if (CryptoCurrency[value.currency] === CryptoCurrency.ETH) {
      return this.conversions.data['1027'].quotes[returnCurrency].price * value.value;
    } else if (CryptoCurrency[value.currency] === CryptoCurrency.WEIS) {
      return this.conversions.data['1027'].quotes[returnCurrency].price * this.weisToEth(value.value);
    }
    return value.value;
  }
